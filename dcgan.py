...
def build_discriminator(inputs):
    ...
    """
    kernel_size = 3
    layer_filters = [32, 32, 64, 128, 256]
...
def train(models, x_train, params):
    ...
    for i in range(train_steps):
        ...
        print('real_images :', real_images.shape[0])
        # generate fake images from noise using generator 
        # generate noise using uniform distribution
        noise = np.random.uniform(-1.0, 1.0, size=[batch_size, latent_size])
        # generate fake images
        fake_images = generator.predict(noise)
        print('fake images : ', fake_images.shape[0])
...
def build_and_train_models():
    input_file = "/home/oleg/Документы/dcgan_my_picture_my_layers/data/labels.txt"
    ifile = open(input_file, 'r')
    allines = ifile.readlines()
    random.shuffle(allines)

	# limitating dataset for simple testing and splitting it
    sample_size=500
    allines_sample=allines[:sample_size]

    percent=0.7
    trainsize = int(0.7 * len(allines_sample))
    testsize = len(allines_sample) - trainsize
    print('trainsize : ', trainsize, 'testsize : ', testsize)
    trainallines = allines_sample[:trainsize]
    testallines = allines_sample[trainsize:]

	# dataset consists not only human pictures,
	# but humans are marked with cls==1
    x_train = []
    for line in trainallines:
        dt = line.strip().split()
        filename = dt[0]
        cls = int(dt[1].strip())
        if (cls==1):
...
if __name__ == '__main__':
    ...
    #generator = load_model('dcgan_muzhik.h5')
    #test_generator(generator)